...
const { data, pending, error } = await useAsyncData(
  `films-dublaj-page-${currentPage.value}`,
  () => $fetch(`/api/films?language_type=turkce-dublaj&page=${currentPage.value}&limit=24`),
  { watch: [currentPage] }
)
...
useSeoMeta({
  title: () => {
    if (currentPage.value > 1) {
      return `Türkçe Dublaj Filmler - Sayfa ${currentPage.value} - FilmİzleJet`
    }
    return 'Türkçe Dublaj Film izle - FilmİzleJet'
  },
  description: () => `Türkçe dublajlı filmler. ${totalFilms.value} dublaj film Full HD kalitesinde izle. En yeni Türkçe dublajlı filmler.`
})
</script>
    ...
    <div class="content-wrapper">
      <div class="content-left">
        <div class="section-header-line">
          <h1 class="section-title">Türkçe Dublaj Filmler</h1>
        ...
        <div v-else-if="films.length === 0" class="empty-state">
          <p>Türkçe dublaj film bulunamadı.</p>
        </div>
