...
const { data, pending, error } = await useAsyncData(
  `films-altyazi-page-${currentPage.value}`,
  () => $fetch(`/api/films?language_type=turkce-altyazi&page=${currentPage.value}&limit=24`),
  { watch: [currentPage] }
)
...
useSeoMeta({
  title: () => {
    if (currentPage.value > 1) {
      return `Altyazılı Filmler - Sayfa ${currentPage.value} - FilmİzleJet`
    }
    return 'Türkçe Altyazılı Film izle - FilmİzleJet'
  },
  description: () => `Türkçe altyazılı filmler. ${totalFilms.value} altyazılı film Full HD kalitesinde izle. En yeni Türkçe altyazılı filmler.`
})
</script>
    ...
    <div class="content-wrapper">
      <div class="content-left">
        <div class="section-header-line">
          <h1 class="section-title">Türkçe Altyazılı Filmler</h1>
